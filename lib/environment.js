...
export function createPublicEnvironment(source = {}) {
  ...
  const useApiProxy = booleanValue(source.NEXT_PUBLIC_USE_API_PROXY ?? source.NEXT_PUBLIC_USE_LOCAL_API_PROXY, 'NEXT_PUBLIC_USE_API_PROXY');
  ...
  return Object.freeze({
    appEnvironment,
    useMockApi,
    useLocalApiProxy: useApiProxy,
    useApiProxy,
    apiUrl: useApiProxy ? '/api/laravel/api/v1' : directApiUrl,
    laravelUrl: useApiProxy ? '/api/laravel' : directLaravelUrl,
  });
}
