...
const ALLOWED_PATTERNS = [
  /^api\/v1\/agencies\/[^/]+\/users$/,
  /^api\/v1\/agencies\/[^/]+\/users\/[^/]+$/,
];

export function normalizedProxyPath(parts = []) {
  const path = parts.map((part) => decodeURIComponent(String(part))).join('/');
  if (!ALLOWED_PATHS.has(path) && !ALLOWED_PATTERNS.some((pattern) => pattern.test(path))) throw new Error('PROXY_PATH_NOT_ALLOWED');
  return path;
}
...
export function rewriteCookieForProxy(cookie, { secure = true } = {}) {
  let rewritten = cookie.replace(/;\s*Domain=[^;]+/gi, '');
  if (!secure) rewritten = rewritten.replace(/;\s*Secure\b/gi, '');
  return rewritten;
}

export function localDevelopmentCookie(cookie) {
  return rewriteCookieForProxy(cookie, { secure: false });
}

export function upstreamSetCookies(headers, options = {}) {
  const cookies = typeof headers.getSetCookie === 'function'
    ? headers.getSetCookie()
    : splitSetCookieHeader(headers.get('set-cookie'));
  return cookies.map((cookie) => rewriteCookieForProxy(cookie, options));
}
