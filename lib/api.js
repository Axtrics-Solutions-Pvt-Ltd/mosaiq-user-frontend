...
export function createAuthApi({
  ...
  return Object.freeze({
    ...
    listWorkspacesForUser: () => request('/auth/workspaces?per_page=100&status=active'),
    updateAgencyUser: async (user, body) => {
      const agencyId = user?.membership?.agency_id;
      const userId = user?.agency_user_id || user?.membership?.agency_user_id || user?.id;
      if (!agencyId || !userId) throw new ApiRequestError('Profile cannot be updated because account access details are missing.', { code: 'PROFILE_UPDATE_UNAVAILABLE' });

      const updateById = (id) => request(`/agencies/${encodeURIComponent(agencyId)}/users/${encodeURIComponent(id)}`, { method: 'PUT', body, stateful: true });
      ...
      const users = await request(`/agencies/${encodeURIComponent(agencyId)}/users?search=${encodeURIComponent(user.email)}&per_page=100`);
      const agencyUser = (users.data || []).find((item) => String(item.email || '').toLowerCase() === String(user.email).toLowerCase());
      if (!agencyUser?.id) {
        throw new ApiRequestError('We could not find your profile record. Please ask the backend developer to include agency_user_id in /auth/me or add a self-profile update endpoint.', { status: 404, code: 'PROFILE_RECORD_NOT_FOUND' });
      }
      return updateById(agencyUser.id);
    },
    ...
    changePassword: (body) => request('/auth/change-password', { method: 'POST', body, stateful: true }),
  });
}
